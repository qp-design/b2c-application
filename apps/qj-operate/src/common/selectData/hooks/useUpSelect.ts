...
export function useUpSelectCommon() {
  ...
  const showSelectValue = (param: string) => {
    const { value, label } = componentOption.find((item) => item.value === param) || {};
    return `${label} (${value})`;
  };

  const showPrevValue = ({ value: param, type }: { value: any; type: dataTypeEum }) => {
    const isStore = type === 'pageStore';
    const resultArray = isStore ? storeOption : queryOption;
    const { value, label } = resultArray.find((item) => item.value === param) || {};
    return `${label} (${value})`;
  };
