...
export const SortList = ({ form, name, parentName, showConfig, delConfig, addConfig }: any) => {
  const monitorInstance = useLowCodeGraph();
  const [list, setList] = useState<Array<any>>(form.getFieldValue('columnList'));
  ...
  const handleSort = (index: number, direction: string) => {
    if ((index === 0 && direction === 'up') || (index === list.length - 1 && direction === 'down')) return;
    setList((prevState) => {
      const result = [...prevState];
      direction === 'up'
          ...
          return (
            <li key={item.id} className={'sortItem'}>
              <div className={'lPart'}>
                ...
                {item.link ? <SelectLink name={[index, 'link']} form={form} parentName={parentName} /> : null}
                <div className={'handleArea'}>
                  ...
                  {showConfig ? (
                    <div className={'hidePart'} onClick={handleDisplay.bind(null, index, item.show)}>
                      {item.show ? '隐藏' : '显示'}
                    </div>
                  ) : null}
                  {delConfig ? (
              ...
              <div className={'rPart'}>
                <SelectPicture form={form} name={[index, 'imgUrl']} parentName={parentName} />
              </div>
            </li>
          );
        })}
