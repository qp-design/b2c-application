...
async function dataImpl(menuOpcode: string) {
  ...
  function computed(
    resultDefault: Array<any>
  ): Array<{ value: string; label: string }> {
    return resultDefault.map(
...
export const useShowParams = (form: FormInstance) => {
  const [, setDefaultOption] = useActionStore(
...
export const { Provider: ActionContextProvider, useStore: useActionStore } =
  createFastContext<Types>(init);
