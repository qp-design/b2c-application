...
import {
  UpDataComponent,
  DownDataComponent
} from '@/common/selectData/components';
...
const WrapInfo = () => {
  ...
  return (
    <>
      ...
      <p>
        上游是组件接入数据，下游是组件输出数据输出到页面store，可以供页面内其他组件使用
      </p>
    </>
  );
};
...
const WrapRoot = (props) => {
  const { openImpl, cancel, confirm, open, dispatchParams, connect } =
    useDataInit(props);
  return (
    <>
